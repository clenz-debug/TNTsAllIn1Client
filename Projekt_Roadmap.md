...
**Phase 7b — Custom Cosmetic Capes (eigener Punkt, nicht Teil von Phase 7)** ⏸ pausiert (2026-09-14)
...
**Offene Frage, die die Umsetzung pausiert:** *wo* das Cape-Bild öffentlich gehostet wird. Erster Ansatz war Backblaze B2 (S3-kompatibel, kostenloses Free-Tier) - stellte sich aber raus, dass Backblaze für **öffentliche** Buckets eine Kreditkarte samt $1-Verifizierungsbuchung verlangt, was der Nutzer vermeiden will. Alternative ohne Karte (GitHub, `raw.githubusercontent.com` + Contents-API) wurde besprochen, aber der Nutzer kennt jemanden mit einem eigenen (nicht gemieteten, bei der Person zuhause stehenden) Server, auf dem bereits ein Minecraft-Server läuft - grundsätzlich also schon aus dem Internet erreichbar. **Bewusst mit Phase 8 zusammen nach hinten geschoben**, da beide dieselbe Art Infrastruktur brauchen (irgendein dauerhaft erreichbarer, selbst kontrollierter Ort für Daten) - falls der Freundes-Server für beides genutzt wird, lohnt sich ein einmaliges, gemeinsames Setup statt zwei getrennter. Wartet auf Antworten des Server-Besitzers zu: Domain oder nur IP? Läuft schon ein Webserver? SSH-Zugang vorhanden? Windows oder Linux? Ist er überhaupt bereit, einen Ordner + eingeschränkten Zugang dafür herzugeben?
