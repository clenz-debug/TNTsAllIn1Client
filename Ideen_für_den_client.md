...
- in Discord soll angezeigt werden das man MC über meinen Client spielt — Timing-Entscheidung 2026-08-10: bewusst spät (nahe Packaging/Phase 9), siehe Chat-Begründung in Aktuelle_Phase.md
  - Design-Entscheidung 2026-09-22 (Timing-Voraussetzung jetzt erfüllt, Logo/Clientname sind final): nicht nur Launcher-seitig, sondern als eigenes Mod-Feature nach demselben Schema wie jedes andere `ClientConfig`-Feature (Haupt-Toggle + Unter-Toggles + eigene Options-Screen wie z.B. `ItemCounterOptionsScreen`, erreichbar über `ClientMenuScreen`) — muss dafür für `mod/1.21.11` und `mod/26.1.2` einzeln gebaut werden, da die beiden unabhängige Projekte sind
  - Haupt-Toggle `discordPresenceEnabled`, Default **aus** (opt-in, nicht automatisch nach einem Update für alle sichtbar)
  - Unter-Toggles (defaulten auf "an", sobald Haupt-Toggle an ist — gleiche Konvention wie der Rest von `ClientConfig`): Singleplayer/Multiplayer-Status anzeigen, Weltname (nur Singleplayer), Minecraft-Version
  - Bei Multiplayer zusätzlich ein eigener, separater Toggle für Server-Name/-Adresse, Default **aus** — anders als der Weltname (betrifft nur einen selbst) würde das öffentlich zeigen, auf welchem Server man zu finden ist
  - Technisch letztlich NICHT jagrosh/DiscordIPC (der obige Plan) — bei der Umsetzung stellte sich heraus, dass die Lib faktisch unmaintained ist (README verweist noch auf JCenter, seit Jahren abgeschaltet), es keine echten Releases gibt und jedes real existierende Mod, das sie trotzdem nutzt, einen beliebigen JitPack-Commit-Hash pinnt. Stattdessen: eigene ~150-Zeilen-Implementierung des Discord-IPC-Protokolls (Named Pipe unter Windows, Unix-Domain-Socket unter Mac/Linux, JSON über Gson - beides ohnehin schon vorhanden, keine neue Abhängigkeit) — weniger Risiko als eine tote Fremd-Lib samt ungeklärter transitiver Abhängigkeiten per Loom jar-in-jar einzubinden
  - [check] (2026-09-22) — für `mod/1.21.11` UND `mod/26.1.2` gebaut, beide kompilieren sauber (`./gradlew compileJava`). SP/MP- und Weltname-Erkennung wiederverwendet exakt dieselbe, bereits bewährte API wie `WaypointScope`. Eigene Discord-Application angelegt (discord.com/developers/applications, "Identität meines Spiels auf Discord anpassen" beim Onboarding gewählt, Social SDK/Verifizierung/Spiele-Bereich/Coverbild allesamt nicht gebraucht), Client-ID in `discordApplicationClientId` eingetragen, `N-Logo-icon-only-1024.png` unter Rich Presence → Art Assets als "logo" hochgeladen, Jar für 26.1.2 gebaut — live bestätigt, "klappt"
  - Fehlerbehebung (2026-09-25): Nach 0.1.0 zeigte Discord im installierten Launcher nichts mehr an. Die Client-ID stand nur von Hand in der `config` einer einzigen Dev-Instanz, frische Instanzen bekamen den Platzhalter `"0"` und verbanden sich stillschweigend nie. Die ID steht jetzt fest im Code (`DiscordPresenceManager#APPLICATION_ID`), das Feld `discordApplicationClientId` ist entfernt (alte Werte in bestehenden Dateien werden ignoriert). Im Dev-Modus bestätigt, "passt"
...
- Gutes Ingame Menü um die featurs / mods auzuwählen (auch über taste(-nkombie) zu öffnen welche mann selbst einstellen kann) [check] (5e)
    ...
    - proximity voice mod oder was ähnliches wäre cool nicht jeder nutzt Discord zum labern das macht es einfacher [später — zu groß für Phase 5, eigene Phase/Bündelung wie 5f prüfen]
      - Entscheidung 2026-09-22: Simple Voice Chat NICHT bündeln — funktioniert automatisch nur in Singleplayer/LAN, auf fremden dedizierten Servern nur wenn der Betreiber dort zusätzlich das Voicechat-Plugin installiert hat (gleicher Grund wie bei Item Physics, siehe Aktuelle_Phase.md: "hätte auf normalen Servern nichts bewirkt"). Wer es will, installiert es sich weiterhin selbst über "Eigene Mods hinzufügen" im Mods-Screen — funktioniert schon heute problemlos
