...
- **5s. 3D skin layers for every player** — no mod code needed, same call as 5p: vanilla renders a skin's second layer (hat, jacket, sleeves, pants) as a flat 2D plane offset slightly outward instead of real geometry with depth. **[3D Skin Layers](https://modrinth.com/mod/3dskinlayers)** (tr7zw) is the de-facto standard mod for this (65M+ downloads), confirmed Sodium-compatible (has its own workaround built in), purely client-side, and applies to every player with a visible second skin layer, not just the local character.
...
**Phase 7b — Custom cosmetic capes (its own item, not part of Phase 7)** ⏸ paused (2026-09-14)
Phase 7 only covers Mojang's own skin/cape system (display + skin upload) — capes there are only ever displayed, never managed. Own request: real, high-resolution custom capes like Lunar/Badlion, uploadable directly in the launcher, visible to other players running a compatible mod. The rendering side is settled: the already-existing, bundled **[Cape Provider](https://modrinth.com/mod/cape-provider)** mod (LGPL-2.1-or-later) handles it via an HTTP GET against a `fabric.mod.json` configuration entry — no custom mixin code needed. The upload-side code (launcher UI, IPC, PNG validation) is also fully built, `npm run typecheck`/`npm run build` green - see `Aktuelle_Phase.md`'s "Custom Capes" section.
**Open question pausing this:** *where* the cape images get publicly hosted. First attempt was Backblaze B2 (S3-compatible, free tier) - turned out Backblaze requires a credit card plus a $1 verification charge for **public** buckets specifically, which the user wants to avoid. A card-free alternative (GitHub, `raw.githubusercontent.com` + Contents API) was discussed, but the user knows someone with their own (not rented, sitting at that person's home) server that already runs a Minecraft server - so it's already internet-reachable in principle. **Deliberately pushed to the end together with Phase 8**, since both need the same kind of infrastructure (some durably-reachable, self-controlled place for data) - if the friend's server ends up serving both, one shared setup beats two separate ones. Waiting on answers from the server owner: domain or bare IP? already running a web server? SSH access? Windows or Linux? and whether they're even willing to hand over a folder + scoped access for this at all.
*Done when:* a custom cape PNG can be uploaded in the launcher and shows up correctly (not stretched/misaligned) for a second player running the bundled Cape Provider mod.

**Phase 8 — Friends/social system (deliberately later, optional, its own mini-project)** — see Phase 7b above: both pushed to the end together, same infrastructure need
...
**Phase 8b — Playing together by invitation (planned 2026-09-25, builds on Phase 8)**
Own request: you only get into a friend's singleplayer world **with an invitation**, not via an open address.
- **Flow:** the host clicks "Invite friends" in the pause menu → picks friends → the invitation goes through our backend (`nxlc.de`, same service as capes/friends) to exactly that friend → they get a notification in the launcher or as an in-game toast with Accept/Decline → on accept, launcher/mod connect automatically, no address to type.
- **Reaching the world:** via a bundled **e4mc** (MIT, Modrinth, available for 1.21.11 and 26.1.2) - "Open to LAN" tunnels the world through e4mc's relay servers and assigns an address `xyz.e4mc.link`. Optionally an own relay on `nxlc.de` later (e4mc already has `brokerUrl`/`relayHost` settings) - only with the server owner's OK, since all game traffic would then go through their connection.
- **Only invited players get in, two layers:** (1) the e4mc address is never shown, the backend only hands it to invitees; (2) the actual lock: our mod on the host checks every joining player's UUID and kicks anyone not invited ("You weren't invited"). The UUID can't be faked since an opened world runs in online mode (Mojang verifies the identity).
- **Invitations** expire (e.g. after 10 minutes or when the host closes the world); the host can revoke them and kick players.
- **Server instead of world:** if the host is on a public server, an invitation just means "come to server.de too" (launch via `--quickPlayMultiplayer` or connect directly) - no lock, the server decides.
- **e4mc check (2026-09-25, version 6.2.1 for 26.1.2):** e4mc stores the address **nowhere retrievable** - it's only in the log and a chat message (translation key `text.e4mc_minecraft.domainAssigned`, address as argument resp. in the copy click). Plan: our mod hooks vanilla's `ChatComponent.addMessage`, recognizes that message by its key, reads the address and hides the message (keeping the address secret). Depends only on vanilla + one translation key, no mixin into e4mc's code - if e4mc changes the key, only inviting stops working (no crash). `text.e4mc_minecraft.closeServer` signals when the world is no longer reachable. e4mc's "PoisonPill" warning only checks the jar's download source on Windows (Modrinth is allowed) - no problem for bundling. e4mc itself restores whitelist/ban commands for LAN worlds; our own UUID check is independent of that.
*Done when:* account A invites account B into their world, B gets in with a click, and an uninvited account C is rejected despite knowing the address.
...
- **Mushrooms Plus (resource pack, Phase 5p follow-up)**: MIT.
- **Vanilla Spinning Stonecutter (3D) (resource pack, Phase 5p follow-up)**: MIT. Adds a real spin animation (via plain vanilla mechanics, no mod needed).
- **Vanilla Tweaks bundle (resource pack, Phase 5p follow-up)**: a custom selection the user assembled themselves on vanillatweaks.net (amethyst, sculk vein, glow lichen, among others), not a fixed Modrinth release. Per vanillatweaks.net/terms/: bundling is allowed but **not unmodified** (the site explicitly requires "modify/enhance" rather than a plain 1:1 pass-through), attribution is mandatory ("Vanilla Tweaks: https://vanillatweaks.net/", on "all main publishing platforms" plus ideally their own `credits.txt`), and **no monetization** of the redistribution (no donation/paywall gating). Not an issue at the project's current non-public hobby stage; **re-check explicitly before any public release** - unlike the other bundles, this one specifically needs to settle whether a plain user selection already counts as "modified enough" or whether a more substantive change would be required. (Blocky 3D Pointed Dripstone, originally bundled for dripstone, was removed again afterward - fully covered by the Vanilla Tweaks bundle with the exact same model scope.)
- **3D Skin Layers (mod, Phase 5s)**: its own "tr7zw Protective License" (a LicenseRef, not an SPDX standard license) - MIT-like at its core (attribution required, copyright notice must be kept), but with an explicit non-commercial clause added ("may not be used for commercial advantage or monetary compensation") - similar in spirit to the Vanilla Tweaks clause above, just for a real mod instead of a resource pack. Shipped unmodified as a jar in `launcher/mods-bundle/` (same treatment as Sodium/Lithium/Continuity), **re-check explicitly before any commercial release**.
...
- **"Third-Party Licenses/Credits" screen** ✅ built (moved up from a later checklist item) - `mod/src/main/java/com/tntsallin1client/menu/CreditsScreen.java` (mod menu, one `ContainerObjectSelectionList` row per entry) and `launcher/src/renderer/src/screens/CreditsScreen.tsx` (launcher, behind a "Credits" button in the Play screen header) - both list the same twelve entries (Fabric Loader/API, Sodium, Lithium, Continuity, every Phase 5p/5r resource pack), each opening its link (mod: `ConfirmLinkScreen`, launcher: a new `shell:open-external` IPC channel). Both lists have to be kept in sync by hand whenever a bundle is added/removed - see the comment in each file.
