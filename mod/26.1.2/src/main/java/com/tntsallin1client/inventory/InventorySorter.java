...
public final class InventorySorter {
	...
	private static void consolidateStacks(MultiPlayerGameMode gameMode, InventoryMenu menu, Player player) {
		for (int target = FIRST_SLOT; target < LAST_SLOT; target++) {
			...
			while (!targetStack.isEmpty() && targetStack.getCount() < targetStack.getMaxStackSize()) {
				...
				click(gameMode, menu, player, source, ContainerInput.PICKUP);
				click(gameMode, menu, player, target, ContainerInput.PICKUP);
	...
	private static void swap(MultiPlayerGameMode gameMode, InventoryMenu menu, Player player, int a, int b) {
		...
		click(gameMode, menu, player, a, ContainerInput.PICKUP);
		click(gameMode, menu, player, b, ContainerInput.PICKUP);
		if (!menu.getCarried().isEmpty()) {
			click(gameMode, menu, player, a, ContainerInput.PICKUP);
		}
	}
	...
	private static void click(MultiPlayerGameMode gameMode, InventoryMenu menu, Player player, int slot, ContainerInput type) {
		gameMode.handleContainerInput(menu.containerId, slot, 0, type, player);
	}
}
