...
import com.tntsallin1client.mixin.AbstractContainerScreenAccessor;
 ...
 *
 * <p><b>Recipe book overlap (follow-up feedback):</b> the button's position used to be
 * re-derived once at init from a hardcoded panel width/height, assuming vanilla always
 * centers the panel - wrong whenever the recipe book is open, since vanilla shifts
 * {@code leftPos} right to make room for it. {@link #repositionSortButton} now reads the
 * panel's real, already-correct layout straight off the screen every tick instead.</p>
 */
public final class QuickSortUi {
	...
	public static void register() {
		ScreenEvents.AFTER_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
			...
			// Positioned every tick instead of just once here - the recipe book (whether it was
			// already open before this screen appeared, or gets toggled while it's up) and the
			// active-effect icon row both shift where vanilla actually draws the panel/icons
			// after AFTER_INIT fires, and neither change re-triggers AFTER_INIT.
			repositionSortButton(containerScreen, sortButton, client);
			ScreenEvents.afterTick(screen).register(scr -> repositionSortButton(containerScreen, sortButton, client));
	...
	/**
	 * Reads the panel's real, already-computed position ({@link AbstractContainerScreenAccessor})
	 * instead of re-guessing a centered one - that guess used to ignore the recipe book shifting
	 * {@code leftPos} to make room for itself, leaving the button misplaced (overlapping the
	 * panel) whenever the book was open.
	 */
	private static void repositionSortButton(AbstractContainerScreen<?> screen, Button sortButton, Minecraft client) {
		AbstractContainerScreenAccessor accessor = (AbstractContainerScreenAccessor) screen;
		int left = accessor.tntsallin1client$getLeftPos();
		int top = accessor.tntsallin1client$getTopPos();
		int imageWidth = accessor.tntsallin1client$getImageWidth();
		int buttonY = top;
	...
	private static void trySort(Minecraft client, Screen screen) {
		if (screen instanceof CreativeModeInventoryScreen creativeScreen && !creativeScreen.isInventoryOpen()) {
			...
			client.player.sendOverlayMessage(Component.translatable("gui.tntsallin1client.sort_button.wrong_tab"));
			return;
		}
		...
		if (!client.player.inventoryMenu.getCarried().isEmpty()) {
			...
			client.player.sendOverlayMessage(Component.translatable("gui.tntsallin1client.sort_button.carried_item"));
			return;
		}
