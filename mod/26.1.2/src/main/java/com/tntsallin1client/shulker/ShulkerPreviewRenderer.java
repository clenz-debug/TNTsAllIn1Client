 ...
 * drawn over, and that only happens once {@code GuiGraphicsExtractor#renderDeferredElements()}
...
public final class ShulkerPreviewRenderer {
	...
	public static void drawIfPending(GuiGraphicsExtractor guiGraphics) {
		...
		guiGraphics.text(font, title, contentX + 2, contentY + HEADER_PADDING / 2, 0xFFFFFFFF);
		...
		for (int index = 0; index < slots.size(); index++) {
			...
			guiGraphics.item(slotStack, slotX, slotY);
			guiGraphics.itemDecorations(font, slotStack, slotX, slotY);
		}
