...
public class ItemCounterHud implements HudElement {
	...
	public static void drawLabel(GuiGraphicsExtractor guiGraphics, Font font, String label, @Nullable ItemStack iconStack,
		...
		if (iconStack != null) {
			...
			guiGraphics.text(font, label, ICON_SIZE + ICON_GAP, textY, color);
		} else {
			guiGraphics.text(font, label, 0, 0, color);
		}
	...
	public static String buildLabel(ClientConfig config, LocalPlayer player) {
		...
		return item.getName(new ItemStack(item)).getString() + ": " + count;
	}
