 ...
 * {@code OptionsSaveMixin}: 26.1.2's {@code Options#save()} validates every option against its
 * ValueSet before writing and logs "Error saving option Brightness: ..." instead of silently
 * accepting the out-of-range bypass value like before - happens on *any* options save while
 * fullbright is active, not just a gamma change. Swapping back to the real value for just the
 * duration of that one call keeps options.txt always valid without weakening the bypass itself.
 */
