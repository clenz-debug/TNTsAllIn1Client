 ...
 *
 * <p>Also reachable from the title screen, before a world is ever joined (see
 * {@code TitleScreenIntegration}) - every bounds method that would otherwise need live player state
 * (position, inventory, equipped gear, ...) falls back to representative placeholder content instead
 * of skipping the entry entirely, so every enabled element can still be positioned up front. See
 * {@link #pinnedRecipeBounds} for the one entry with two independent placeholder fallbacks (recipe
 * pinned but no player, and nothing pinned at all yet).
 */
public class HudEditorScreen extends Screen {
	...
	/** Representative ingredient-row length for {@link #pinnedRecipeBounds} when nothing has ever
	 * been pinned yet - no real recipe to size from, so this just needs to be plausible enough to
	 * position the box against (most recipes land in the 2-4 distinct-ingredient range). */
	private static final int PLACEHOLDER_INGREDIENT_COUNT = 3;
	...
	public void extractRenderState(net.minecraft.client.gui.GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
		...
		for (Entry entry : entries) {
			...
			guiGraphics.fill(
					...
					0xFFFFFFFF);
			guiGraphics.text(this.font, entry.label, bounds.x, bounds.y - this.font.lineHeight - 2, 0xFFFFFF55);
		}

		guiGraphics.centeredText(this.font, this.title, this.width / 2, 6, 0xFFFFFFFF);
		guiGraphics.centeredText(this.font, Component.translatable("gui.tntsallin1client.hud_editor.hint"), this.width / 2, 6 + this.font.lineHeight + 2, 0xFFFFFFFF);
	}
	...
	private Rect armorStatusBundledBounds() {
		// No player needed - buildMaxEntries only depends on which slots are enabled, sized as if
