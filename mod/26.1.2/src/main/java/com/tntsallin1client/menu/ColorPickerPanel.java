...
public class ColorPickerPanel {
	...
	public ColorPickerPanel(Font font, int x, int y, int width, int initialArgb,
		...
		setPatternFilteredResponder(this.redField, "[0-9]{0,3}", v -> onChannelEdited());
		setPatternFilteredResponder(this.greenField, "[0-9]{0,3}", v -> onChannelEdited());
		setPatternFilteredResponder(this.blueField, "[0-9]{0,3}", v -> onChannelEdited());
		setPatternFilteredResponder(this.hexField, "[0-9a-fA-F]{0,6}", this::onHexEdited);
	...
	public void render(GuiGraphicsExtractor guiGraphics, int labelColor) {
	...
	/**
	 * {@code EditBox#setFilter} no longer exists (removed alongside the preedit/IME rework) -
	 * rebuilds the same "reject keystrokes that don't match" behavior on top of {@code setResponder}
	 * by reverting to the last valid value instead, reusing {@link #suppressResponders} the same way
	 * {@link #pushColor} already does to avoid re-triggering {@code onValid} on the revert itself.
	 */
	private void setPatternFilteredResponder(EditBox field, String regex, Consumer<String> onValid) {
	...
	private static void drawMarker(GuiGraphicsExtractor guiGraphics, int centerX, int centerY) {
