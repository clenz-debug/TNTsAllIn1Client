...
public class ClientModsCardScreen extends Screen implements FeatureSink {
	...
	private void renderCard(GuiGraphicsExtractor graphics, Card card, int x, int y, int hoverY) {
		...
		if (!FeatureIcons.has(card.key)) {
			...
			graphics.text(this.font, title, x + (CARD_WIDTH - this.font.width(title)) / 2, titleY, theme.text, false);
		}
		...
		graphics.text(this.font, options, x + (CARD_WIDTH - this.font.width(options)) / 2, optionsTop + (OPTIONS_HEIGHT - this.font.lineHeight) / 2,
	 ...
	 */
	private void renderLargeName(GuiGraphicsExtractor graphics, String name, int x, int y) {
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			graphics.text(this.font, line, -this.font.width(line) / 2, 0, ClientTheme.get().text, false);
