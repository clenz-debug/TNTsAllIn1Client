...
public class PinnedRecipeListScreen extends Screen {
	...
	private static final class RecipeList extends ContainerObjectSelectionList<RecipeList.Row> {
		...
		static final class Row extends ContainerObjectSelectionList.Entry<Row> {
			...
			/** Recomputed fresh every {@link #extractContent} call instead of cached at construction -
			...
			static Row of(Font font, PinnedRecipe recipe, PinnedRecipeListScreen owner) {
				...
				// No character-level filter here (unlike the 1.21.11 build of this same class) - this
				// version's EditBox dropped that API entirely; setCraftMultiplierText below already
				// tolerates and ignores non-numeric input instead of crashing on it, so this is still
				// correct, just without blocking invalid keystrokes outright.
			...
			public void extractContent(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, boolean hovered, float partialTick) {
				...
				this.qtyBox.extractRenderState(guiGraphics, mouseX, mouseY, partialTick);
				...
				this.visibleToggle.extractRenderState(guiGraphics, mouseX, mouseY, partialTick);

				int removeX = toggleX + VISIBLE_TOGGLE_WIDTH + WIDGET_GAP;
				this.removeButton.setPosition(removeX, this.getContentY());
				this.removeButton.extractRenderState(guiGraphics, mouseX, mouseY, partialTick);
			}
