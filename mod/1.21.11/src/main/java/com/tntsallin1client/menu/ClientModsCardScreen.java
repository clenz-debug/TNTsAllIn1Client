...
public class ClientModsCardScreen extends Screen implements FeatureSink {
	...
	private void renderCard(GuiGraphics graphics, Card card, int x, int y, int hoverY) {
		...
		graphics.drawString(this.font, title, x + (CARD_WIDTH - this.font.width(title)) / 2, y + (TITLE_HEIGHT - this.font.lineHeight) / 2, theme.text, false);
		...
		if (card.options != null) {
			...
			graphics.drawString(this.font, hint, x + (CARD_WIDTH - this.font.width(hint)) / 2, imageTop + (IMAGE_HEIGHT - this.font.lineHeight) / 2,
