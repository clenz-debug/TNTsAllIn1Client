...
**Phase 5 — Eigene Features, leicht → schwer** 🔧 in Arbeit — 5a–5f und 5p abgeschlossen und alle live vom Nutzer bestätigt (5d ist bewusst nur ein erster Wurf, siehe unten); 5g/5h/5i implementiert und gebaut, wartet auf Live-Bestätigung; 5j–5o noch nicht begonnen (siehe Roadmap)
...
**5h — Zoom (`zoom/ZoomHandler.java`, `keybind/ModKeyBindings.ZOOM`):** Kein Mixin nötig — `GameRenderer#getFov()` liest `options.fov()` jeden Frame frisch aus (in den decompilierten Quellen verifiziert), also reicht es, den Options-Wert bei gehaltener Taste temporär zu überschreiben und beim Loslassen wiederherzustellen; Vanillas eigenes FOV-Lerp/Smoothing übernimmt den Rest. Ruft nie `Options#save()`, landet also nie in `options.txt`. Taste standardmäßig unbelegt (gleiches Muster wie Inventar-Sortierung/Mod-Menü-Taste), zusätzlich per Mod-Menü-Toggle komplett abschaltbar.
...
## Nächster Schritt
Phase 4 ist abgeschlossen, Phase 5 (5a–5f, 5p) ist abgeschlossen und live bestätigt — Phase 5 bleibt aber offen für einen zweiten Durchgang (5g–5o). Offene Punkte, unabhängig voneinander:
1. **5j–5o umsetzen** (siehe Roadmap-Phase-5-Abschnitt, aus `Ideen_für_den_client.md` nachgezogen): Fullbright/Lightlevel-Overlay, Shulkerbox-Anzeige, Hitbox-Farbe, Keystrokes-Anzeige, Screenshot-Toast, Itemphysics — wieder leicht → schwer wie beim ersten Durchgang, jedes Feature einzeln togglebar. (5g FPS-Anzeige, 5h Zoom, 5i Fadenkreuz bereits umgesetzt und gebaut, siehe oben.)
2. **5g/5h/5i live testen** (`runClient` reicht, wie bei 5a–5f) und dabei entscheiden, ob die Launcher-Instanz jetzt auch aktualisiert werden soll (neuer Mod-Jar + fehlendes Continuity nachziehen) oder ob `runClient` weiterhin der primäre Testweg bleibt.
3. **Phase 3 abschließen:** weiter warten auf Antwort zum (erneut eingereichten, 2026-07-18) Minecraft-API-Freischaltungsantrag (`aka.ms/mce-reviewappid`). Sobald sie da ist: `completeMinecraftLogin` in `launcher/src/main/auth/minecraftAuth.ts` real ohne Mock-Fallback testen (einfach nochmal "Mit Microsoft anmelden" klicken — kein "Dev-Mock-Profil"-Badge mehr sollte dann erscheinen), Phase 3 als ✅ markieren.
4. **Phase 6 angehen** (Launcher-UX: Versionsauswahl & Mod-Verwaltung, siehe Roadmap) — Versions-Picker, persistente Instanz-Konfiguration, Mods-Screen (gebündelte Mods an/ausschalten + eigene Fabric-Mod-Jars hinzufügen). Sollte dabei auch gleich die manuelle resourcepacks-Kopie aus 5p automatisieren. Blockiert nicht durch das Phase-3-Warten.
5. **5d optional nachschärfen**, sobald der Nutzer konkreteres Feedback von der Person hat, die das F3-Feature ursprünglich vorgeschlagen hat.
6. **Phase 7+ / Lizenz-Punkte im Hinterkopf behalten:** durch 5f ist jetzt zum ersten Mal ein fremder Mod (Continuity, LGPL-3.0-only) gebündelt, durch 5p zum ersten Mal ein volles GPL-3.0-Asset (3D Default, siehe Lizenz-Hinweis oben) — der laut Roadmap ohnehin geplante "Third-Party Licenses/Credits"-Screen (Mod + Launcher) hat damit zwei echte Einträge. Falls 5o (Itemphysics) ebenfalls über einen gebündelten Fremd-Mod gelöst wird, dessen Lizenz vorher genauso prüfen.
7. **Nicht in Phase 5 aufgenommen, für später/Ende vorgemerkt:** Farbschema-Auswahl, Willkommens-/Setup-Wizard beim ersten Start, Discord Rich Presence, Cross-Platform-Kompatibilität, Auto-Update — das sind Launcher-/Cross-Cutting-Themen, keine Mod-Features (siehe Roadmap-Phasen 6/8/9). Proximity Voice Chat ist bewusst außen vor: zu groß für "leicht bis schwer" (eigenes Backend/Audio-Pipeline nötig, siehe Phase 8) — Kandidat für eine eigene spätere Phase oder Bündelung eines bestehenden Mods (z.B. Simple Voice Chat) nach dem 5f-Muster.
