...
**Phase 5 — Eigene Features, leicht → schwer** 🔧 in Arbeit — 5a–5f und 5p abgeschlossen und alle live vom Nutzer bestätigt (5d ist bewusst nur ein erster Wurf, siehe unten); 5g/5h implementiert und gebaut, wartet auf Live-Bestätigung; 5i–5o noch nicht begonnen (siehe Roadmap)
...
1. **5i–5o umsetzen** (siehe Roadmap-Phase-5-Abschnitt, aus `Ideen_für_den_client.md` nachgezogen): Custom Crosshair, Fullbright/Lightlevel-Overlay, Shulkerbox-Anzeige, Hitbox-Farbe, Keystrokes-Anzeige, Screenshot-Toast, Itemphysics — wieder leicht → schwer wie beim ersten Durchgang, jedes Feature einzeln togglebar. (5g FPS-Anzeige, 5h Zoom bereits umgesetzt und gebaut, siehe oben.)
2. **5g/5h live testen** (`runClient` reicht, wie bei 5a–5f) und dabei entscheiden, ob die Launcher-Instanz jetzt auch aktualisiert werden soll (neuer Mod-Jar + fehlendes Continuity nachziehen) oder ob `runClient` weiterhin der primäre Testweg bleibt.
